...
/*   Updated: 2016/06/14 18:16:20 by jnaddef          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/corewar.h"

static void	verbose_ldi2(t_list *param, t_process *proc)
{
	ft_putstr(" ");
	...
	ft_putnbr(proc->pc + (((t_param*)param->content)->
		real_value + ((t_param*)param->next->content)->real_value) % IDX_MOD);
	ft_putstr(")\n");
}

static void	verbose_ldi(t_list *param, t_process *proc)
{
	char	*str;
	int		len;
