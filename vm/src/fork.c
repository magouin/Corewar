...
/*   Updated: 2016/06/15 14:55:21 by jnaddef          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/corewar.h"

static void	verb_fork(t_env *env, t_process *par)
{
	char	*str;
	int		len;
	int		data;
	...
	ft_putnbr(data);
	ft_putstr(" (");
	ft_putnbr((par->pc + data % IDX_MOD) % MEM_SIZE);
	ft_putstr(")\n");
	free(str);
}

void		ft_fork(t_env *env, t_process *par)
{
	t_process	*new;
	int			x;
	...
	new->pc = ft_mod((par->pc + ((short)ft_getnumber(env->arena
						, par->pc + 1, IND_SIZE)) % IDX_MOD) % MEM_SIZE);
